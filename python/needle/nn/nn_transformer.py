...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        if self.causal:
            mask = self.create_causal_mask(queries_len, keys_values_len, device=self.device).broadcast_to(probs.shape)
            probs = probs + Tensor(mask, device=self.device, dtype=self.dtype, requires_grad=False)
...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        x_flat = x.reshape((batch_size*seq_len, x_dim))

        x = x + self.dropout(self.linear2(self.dropout(self.relu(self.linear1(self.norm(x_flat)))))).reshape((batch_size, seq_len, x_dim))
        ### END YOUR SOLUTION
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        transformer_layers = [
            TransformerLayer(embedding_size,num_head=num_head,dim_head=dim_head,hidden_size=hidden_size,
                            dropout=dropout,causal=causal,device=device,dtype=dtype
            ) for _ in range(num_layers)
        ]
        self.transformer_layers = Sequential(*transformer_layers)
        ### END YOUR SOLUTION
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        positions = [i for i in range(x.shape[1])]
        positions = ndarray.array(positions, device=self.device).reshape((x.shape[1],1)).broadcast_to((x.shape[1], x.shape[0]))
        positions = Tensor(positions, device=self.device, dtype=self.dtype)
        posisiton_embedding = self.embedding(positions).transpose(axes=(0, 1))
        x = x + posisiton_embedding
